...
type LibAnalysisResult struct {
	Name           string
	Version        string   // 库版本（从库内 version 头/宏提取；空表示未检测到）
...
// versionMacroRe 匹配常见的版本宏定义：`#define XXX_VERSION_MAJOR 1` / `_VERSION_MINOR` / `_VERSION_PATCH`
var versionMacroRe = regexp.MustCompile(`#define\s+([A-Za-z0-9_]*[Vv]ERSION_?(MAJOR|MINOR|PATCH))\s+([0-9]+)`)

// detectLibraryVersion 从库目录内的头文件提取版本号。
// 匹配 `<LIB>_VERSION_MAJOR/MINOR/PATCH` 三连宏（如 WEBVIEW_VERSION_MAJOR 0 / MINOR 12 / PATCH 1）；
// 同时回退单宏 `VERSION` / `<LIB>_VERSION` 形式。找不到返回空串。
func detectLibraryVersion(pkgDir string) string {
	...
	_ = walkPkgTree(pkgDir, 10, func(absPath, relPath string, isDir bool) {
		...
		for _, m := range versionMacroRe.FindAllSubmatch(data, -1) {
			comp := strings.ToUpper(string(m[2]))
			val := string(m[3])
			switch comp {
			case "MAJOR":
				if major == "" {
		...
		// 单宏回退：`#define <LIB>_VERSION "1.2.3"` 或 `#define VERSION 5`
		if single == "" {
			if m := regexp.MustCompile(`#define\s+[A-Za-z0-9_]*VERSION\s+"([0-9][0-9.]*)"`).FindSubmatch(data); len(m) == 2 {
				single = string(m[1])
			}
		}
	})
...
func AnalyzePackage(pkgDir string) (*LibAnalysisResult, error) {
	...
	return &LibAnalysisResult{
		Name:           libName,
		Version:        detectLibraryVersion(pkgDir),
		Type:           libType,
		Header:         mainHeader,
		Headers:        headers,
