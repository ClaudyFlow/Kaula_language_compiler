...
func newCodeGeneratorForTest(t *testing.T) *CodeGenerator {
	t.Helper()
	// go test ./internal/codegen → CWD = <repo>/compiler/internal/codegen
	// 需要找到 <repo>/compiler/templates 和 stdlib.json：连续上溯寻找有 templates/main.c.tmpl 的目录
	candidates := []string{
		filepath.Join("..", ".."),                      // 从 internal/codegen 上两级 -> compiler
		".",                                            // 已在 compiler 根
		filepath.Join("..", "..", "..", "compiler"), // 从 repo/kaula 目录上溯
	}
	var foundDir string
