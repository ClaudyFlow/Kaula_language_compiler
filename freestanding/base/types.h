#ifndef KAULA_FREESTANDING_BASE_TYPES_H
#define KAULA_FREESTANDING_BASE_TYPES_H

// freestanding/base/types.h — 基础数据类型
// 与 std/base/types.h 保持一致，但不依赖 <sys/types.h> 等 OS 头文件
// 仅使用编译器在 freestanding 模式下也提供的基础头 <stdint.h>/<stddef.h>/<stdbool.h>

#include <stdint.h>
#include <stdbool.h>
#include <stddef.h>

// 平台检测宏（freestanding 下仅区分 Windows/非 Windows，不引入任何 OS 头）
#if defined(_WIN32) || defined(_WIN64) || defined(__CYGWIN__)
    #define FS_PLATFORM_WINDOWS 1
    #define FS_PLATFORM_UNIX 0
#else
    #define FS_PLATFORM_WINDOWS 0
    #define FS_PLATFORM_UNIX 1
#endif
...
// ssize_t — freestanding 无 <sys/types.h>，用指针宽度有符号类型替代
// （与 std/base/types.h 在 Windows 上的处理一致）
#if !defined(KAULA_FS_SSIZE_T_DEFINED) && !defined(_SSIZE_T_DEFINED) && \
    !defined(__ssize_t_defined) && !defined(_SSIZE_T_)
#define KAULA_FS_SSIZE_T_DEFINED
typedef intptr_t ssize_t;
#endif

// 带长度前缀的字符串类型（与 std/base/types.h 同定义、同保护宏）
...
// 类型转换函数（实现见 types.c，弱符号，托管模式下由 std.base 的强符号覆盖）
...
#endif /* KAULA_FREESTANDING_BASE_TYPES_H */
